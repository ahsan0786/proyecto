...
mysql --host $SLAVE_IP -uroot -p$MYSQL_SLAVE_PASSWORD -AN -e 'STOP SLAVE;';
mysql --host $SLAVE_IP -uroot -p$MYSQL_MASTER_PASSWORD -AN -e 'RESET SLAVE ALL;';

mysql --host $MASTER_IP -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "CREATE USER '$MYSQL_REPLICATION_USER'@'%';"
mysql --host $MASTER_IP -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "GRANT REPLICATION SLAVE ON *.* TO '$MYSQL_REPLICATION_USER'@'%' IDENTIFIED BY '$MYSQL_REPLICATION_PASSWORD';"
mysql --host $MASTER_IP -uroot -p$MYSQL_MASTER_PASSWORD -AN -e 'flush privileges;'
...
echo $MASTER_IP
mysql --host $SLAVE_IP -uroot -p$MYSQL_SLAVE_PASSWORD -AN -e "CHANGE MASTER TO master_host='mysqlmaster', master_port=3306, \
...
echo $SLAVE_IP
mysql --host $MASTER_IP -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "CHANGE MASTER TO master_host='mysqlslave', master_port=3306, \
...
mysql --host $SLAVE_IP -uroot -p$MYSQL_SLAVE_PASSWORD -AN -e "start slave;"
